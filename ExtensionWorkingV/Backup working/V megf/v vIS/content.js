(function () {
  ...
  const BLOCKED_WORDS = [
    ...
    "Summary", "Select", "Temperature", "backusing"
  ];
  ...
  restoreBtn.style.cssText = `
    position: fixed !important; top: 10px; right: 10px; z-index: 100000;
...
`;
  ...
  minimizeBtn.style.cssText = `background: none; color: white; border: none; font-size: 18px; cursor: pointer;`;

  const closeBtn = document.createElement("button");
  closeBtn.textContent = "×";
  closeBtn.title = "Clear content";
  closeBtn.style.cssText = `background: none; color: white; border: none; font-size: 18px; cursor: pointer;`;
  ...
  header.onmousedown = function (e) {
    if (e.target.tagName === "INPUT") return; // Let typing happen
    e.preventDefault();
  ...
  function fetchFromGitHub(word) {
    const url = `https://bimaadizi.github.io/test/${encodeURIComponent(word)}.html`;
    contentArea.innerHTML = `<em style="color:gray">Loading "${word}" from GitHub Pages...</em>`;
    ...
    fetch(url)
      ...
      .then(html => {
        contentArea.innerHTML = `<strong>${word}</strong><br><br>` + html;
      ...
      .catch(err => {
        contentArea.innerHTML = `<em style="color:red">❌ No entry found for "${word}"</em>`;
        console.error(err);
      });
  }
...
document.addEventListener("keydown", function (e) {
  // Don't interfere with typing in search bar or other inputs
  ...
  if (e.key.toLowerCase() === "x") {
    e.preventDefault();
    closeBtn.click(); // Clear content
  }

  if (e.key.toLowerCase() === "m") {
    e.preventDefault();
    if (minimized) {
      restoreBtn.click(); // Maximize
    } else {
